...
{
    /// <summary>
    /// The sound recorder to wav format.
    /// </summary>
    public class WaveRecorder
    {
        ...
        /// 
        /// </summary>
        /// <param name="Sample_R">This sample will be used if mono</param>
        /// <param name="Sample_L"></param>
        public void AddSample(int Sample_R, int Sample_L)
        {
            if (!_IsRecording)
                return;
            switch (channels)
                ...
                case 1:
                    {
                        switch (bitsPerSample)
                        {
                            case 08: AddSample_mono_08(Sample_R); break;
                            case 16: AddSample_mono_16(Sample_R); break;
                            case 24: AddSample_mono_24(Sample_R); break;
                            case 32: AddSample_mono_32(Sample_R); break;
                        }
                        break;
                    }
                case 2:
                    {
                        switch (bitsPerSample)
                        {
                            case 08: AddSample_stereo_08(Sample_R, Sample_L); break;
                            case 16: AddSample_stereo_16(Sample_R, Sample_L); break;
                            case 24: AddSample_stereo_24(Sample_R, Sample_L); break;
                            case 32: AddSample_stereo_32(Sample_R, Sample_L); break;
                        }
                        break;
                    }
            }
        ...
        private void AddSample_stereo_08(int Sample_R, int Sample_L)
        {
            STR.WriteByte((byte)(Sample_R & 0xFF));
            // Same sample twice
            STR.WriteByte((byte)(Sample_L & 0xFF));
        }
        private void AddSample_stereo_16(int Sample_R, int Sample_L)
        {
            STR.WriteByte((byte)(Sample_R & 0xFF));
            STR.WriteByte((byte)((Sample_R & 0xFF00) >> 8));

            STR.WriteByte((byte)(Sample_L & 0xFF));
            STR.WriteByte((byte)((Sample_L & 0xFF00) >> 8));

        }
        private void AddSample_stereo_24(int Sample_R, int Sample_L)
        {
            STR.WriteByte((byte)((Sample_R & 0x000000FF) >> 00));
            STR.WriteByte((byte)((Sample_R & 0x0000FF00) >> 08));
            STR.WriteByte((byte)((Sample_R & 0x00FF0000) >> 16));

            STR.WriteByte((byte)((Sample_L & 0x000000FF) >> 00));
            STR.WriteByte((byte)((Sample_L & 0x0000FF00) >> 08));
            STR.WriteByte((byte)((Sample_L & 0x00FF0000) >> 16));
        }
        private void AddSample_stereo_32(int Sample_R, int Sample_L)
        {
            STR.WriteByte((byte)((Sample_R & 0x000000FF) >> 00));
            STR.WriteByte((byte)((Sample_R & 0x0000FF00) >> 08));
            STR.WriteByte((byte)((Sample_R & 0x00FF0000) >> 16));
            STR.WriteByte((byte)((Sample_R & 0xFF000000) >> 24));


            STR.WriteByte((byte)((Sample_L & 0x000000FF) >> 00));
            STR.WriteByte((byte)((Sample_L & 0x0000FF00) >> 08));
            STR.WriteByte((byte)((Sample_L & 0x00FF0000) >> 16));
            STR.WriteByte((byte)((Sample_L & 0xFF000000) >> 24));
        }
        public void Stop()
        {
            if (_IsRecording & STR != null)
            {
                ...
                // Update Chunck Size (0 currently in the header) with data recorded
                SIZE = NoOfSamples + 36;
